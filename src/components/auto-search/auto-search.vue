...
import { adminMassageAS} from "@/api/system";
import {searchAdminOtherName} from "@/api/member";
export default {
  props: {
    disabled: {
      type: Boolean,
  ...
  methods: {
    ...
    onSearch() {
      ...
      } else if(this.stage=='adminUser'){
        let postData={
          userName:this.value
        }
        adminMassageAS(postData).then(res=>{
          if(res.data.length>0){
            this.data=res.data;
            this.tip=2;
          }else{
      ...
      }else if(this.stage=='nickName'){
        let postData={
          other_name:this.value
        }
        searchAdminOtherName(postData).then(res=>{
          if(res.data.length>0){
              this.data=res.data;
            this.tip=2;
          }else{
    ...
    onSelect(value) {
      ...
      this.data.forEach(item => {
        if (item.userName == value) {
          ...
          this.$emit('sendUserName',item.userName)
