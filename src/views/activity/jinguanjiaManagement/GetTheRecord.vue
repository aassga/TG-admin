<template>
    <div v-if="selects.length">
            ...
            <Form ref="searchKey" inline class="pb10">
                <FormItem label="金管家类型">
                    <Select v-model="searchKey.classId" class="w120x">
                        <Option v-for="(item, i) in selects" :key="'level' + i" :value="item.id" v-if="item.id!=0">{{ item.class_name }}</Option>
                    </Select>
                </FormItem>
                <FormItem label="金管家等级">
                    <Select v-model="searchKey.level" class="s-86" clearable placeholder="全部">
                        <Option v-for="(item, i) in levelList" :key="'level' + i" :value="item.value">{{ item.name }}</Option>
                    </Select>
                </FormItem>
                <FormItem label="领取状态">
                ...
                <Select class="w110x mr20" v-if="searchKey.type == 'invite_gift'" v-model="searchKey.contentType">
                    <Option value="invite_userName">邀请账号</Option>
                    <Option value="userName">好友账号</Option>
                </Select>
                <FormItem :label="searchKey.type == 'invite_gift' ? '' : '会员账号'">
                    <Input placeholder="多个请以,逗号隔开" @keyup.enter.native="searchSubmit" class="s-160" clearable v-model="searchKey.userName" />
                </FormItem>
                <FormItem>
        ...
        <div class="v2-search bb">
            <span v-for="item in modeBtns" class="modeBtns ib cs ft16" @click="changeType(item)" :class="searchKey.type == item.type ? 'active' : ''" :key="item.type">{{
                item.name
            }}</span>
        </div>
        <div class="v2-table">
...
export default {
    ...
    data() {
            ...
            columns: [
                ...
                },
                {
                    ...
                    renderHeader: (h, params) => {
                        let title = this.searchKey.type == 'week_gift' ? '周俸禄' : this.searchKey.type == 'month_gift' ? '月俸禄' : '晋级礼金';
                ...
                },
                {
                    ...
                    className: 'pl10 pr10',
                    render: (h, params) => {
                        return <span>{Number(params.row.gift_money_requirement.payTotal).toFixed(2)}</span>;
                    }
                },
                {
                    title: '当周打码',
                    ...
                    className: 'pl10 pr10',
                    render: (h, params) => {
                        return <span>{Number(params.row.gift_money_requirement.betTotal).toFixed(2)}</span>;
                    }
                },
                {
                    title: '当月存款',
                ...
                },
                {
                    ...
                    minWidth: 80,
                    render: (h, params) => {
                        let color = params.row.userName == '总计' ? '#666' : params.row.gift_money_receive == 'yes' ? 'red' : '#4FC3F7';
                        return <span style={{ color }}>{params.row.gift_money_receive == 'yes' ? '已领取' : params.row.userName == '总计' ? '-' : '未领取'}</span>;
                    }
                },
                {
                    title: '更新时间',
                    ...
                    minWidth: 130,
                    render: (h, params) => {
                        return <span>{params.row.userName == '总计' ? '-' : this.$root.newUnixTime(params.row.gift_money_receive_time)}</span>;
                    }
                },
                {
                    title: '领取时间',
                    ...
                    children: [
                        {
                            ...
                            minWidth: 100,
                            render: (h, params) => {
                                return <span>{params.row.invite_userName == '总计' ? '-' : params.row.gift_money_requirement.invite_gift_money_percentage + '%'}</span>;
                            }
                        },
                        {
                            title: '奖励金额',
                            ...
                            minWidth: 100,
                            render: (h, params) => {
                                return <span>{params.row.gift_money_requirement.invite_gift_money}</span>;
                            }
                        },
                        {
                            title: '存款金额',
                            ...
                            minWidth: 100,
                            render: (h, params) => {
                                return <span>{Number(params.row.gift_money_requirement.get_invite_gift_money_of_member_pay).toFixed(2)}</span>;
                            }
                        },
                        {
                            title: '打码量',
                            ...
                            minWidth: 100,
                            render: (h, params) => {
                                return <span>{Number(params.row.gift_money_requirement.get_invite_gift_money_of_member_bet).toFixed(2)}</span>;
                            }
                        }
                    ]
                },
                    ...
                    children: [
                        {
                            ...
                            minWidth: 80,
                            render: (h, params) => {
                                let color = params.row.invite_userName == '总计' ? '#666' : params.row.gift_money_receive == 'yes' ? 'red' : '#4FC3F7';
                                return <span style={{ color }}>{params.row.invite_userName == '总计' ? '-' : params.row.gift_money_receive == 'yes' ? '已领取' : '未领取'}</span>;
                            }
                        }
                    ]
                },
                    ...
                    children: [
                        {
                            ...
                            minWidth: 130,
                            render: (h, params) => {
                                return <span>{params.row.invite_userName == '总计' ? '-' : this.$root.newUnixTime(params.row.gift_money_receive_time)}</span>;
                            }
                        },
                        {
                    title: '领取时间',
     ...
     beforeRouteEnter(tos, from, next){
            next(vm=>{
                if(vm.chessList.includes(vm.$root.nowSite.id)||vm.$root.modeName()=="mode_e"||vm.$root.modeName()=="no"){
                    vm.$router.push({path:'/activity/housekeeper/GetTheRecordX'})
                }
    ...
    methods: {
                    ...
                    if (res && res.code == 200) {
                        ...
                        list.length > 0 &&
                            list.forEach(t => {
                                this.totalList.push({
                                    ...
                                    payCount: t.payCount,
                                    payTotal: t.payTotal,
                                    get_invite_gift_money_of_member_pay: t.get_invite_gift_money_of_member_pay,
                                    get_invite_gift_money_of_member_bet: t.get_invite_gift_money_of_member_bet
                                });
                            });
                        ...
                        if (this.searchKey.type != 'level_gift') {
                            this.table.data.map(item => {
                                item.gift_money_requirement = JSON.parse(item.gift_money_requirement);
                            });
                        }
        ...
        createTotalData(total) {
            if (this.searchKey.type == 'level_gift') {
                ...
                return item;
            } else if (this.searchKey.type == 'week_gift' || this.searchKey.type == 'month_gift') {
                const { bet_money, gift_money, betTotal, payTotal, payCount, invite_gift_money } = total;
                let item = {
                    userName: '总计',
                    level: '-',
                    bet_money,
                ...
                item.gift_money_requirement = JSON.stringify(item.gift_money_requirement);
                return item;
            } else if (this.searchKey.type == 'invite_gift') {
                const { bet_money, gift_money, betTotal, payTotal, payCount, invite_gift_money, get_invite_gift_money_of_member_pay, get_invite_gift_money_of_member_bet } = total;
                let item = {
                    invite_userName: '总计',
                    userName: '-',
                    level: '-',
        ...
        setColumns(val) {
            ...
            if (val == 'level_gift') {
                ...
                this.table.columns = this.columns.filter(item => {
                    ...
                    return item.title != '晋级礼金' && item.title != '月俸禄' && item.title != '当月打码' && item.title != '当月存款';
                });
            } else if (val == 'month_gift') {
                this.table.columns = this.columns.filter(item => {
                    //return item.title != '晋级礼金' && item.title != '周俸禄' && item.title != '周俸禄达标情况';
                    return item.title != '晋级礼金' && item.title != '月俸禄' && item.title != '当周打码' && item.title != '当周存款';
                });
            } else if (val == 'invite_gift') {
