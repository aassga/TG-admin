            ...
            >
             ...
             <div slot="content">
                  <Form :label-width="220" ref="formKey" :model="formKey" :rules="rules">
                       <FormItem label="类别" >
                          <!-- <Select class="w400x"  v-model="formKey.classId" >
...
export default {
    ...
    beforeRouteEnter(to, from, next){
        let siteId = JSON.parse(localStorage.nowSite).id;
        next(vm=>{  
        if(siteId ==63 || siteId == 25 || siteId == 41 || siteId == 42 || siteId == 43 || siteId == 70||siteId==28||siteId==1||vm.$root.modeName()=="no"){
           
                vm.$router.push({path:'/activity/housekeeper/codeManagementXX'})
           ...
           addSubmit(){
            ...
            // this.$refs.formKey.validate(valid=>{
                // if(valid){
                    ...
                    let data={...this.formKey,siteId:this.$root.nowSite.id,classId:this.searchKey.classId};
