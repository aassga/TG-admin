<template>
  <div>
    ...
    <div class="v2-search bb mt20" v-if="currentModeId&&!spinShow">
      <span
      ...
      >
        ...
        <Icon
          size="18"
          v-if="isEdit && currentModeId == item.id"
          class="close ml10"
          color="#888"
    ...
    <div v-if="!spinShow && !currentModeId" class="txt-a" style="line-height:200px">
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    ...
    if (vm.chessList.includes(vm.$root.nowSite.id)||vm.$root.modeName()=="mode_e"||vm.$root.modeName()=="no") {
        vm.$router.push({ path:"/activity/housekeeper/levelManagementXX" });
      }
    });
    next();
  },
      ...
      jinGuanJiaClassList(data).then(res => {
        if (res && res.code) {
          ...
          this.modeBtns.forEach(t=>{
            if(t.model_name!="邀请者奖励"){
             this.modeBtnsNoChess.push(t);
            }
          })
          this.modeName = [];
          this.modeBtns.map(t => {
