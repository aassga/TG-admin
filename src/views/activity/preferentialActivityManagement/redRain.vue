<template>
    <div>
        ...
        <div class="v2-search">
            <Form inline ref="searchKey">
                <FormItem v-if="checkedArr.length > 0">
                    <Button type="primary" @click.prevent="cancelStatys('allCancel', 'no')">取消资格</Button>
                </FormItem>
                <FormItem v-if="checkedArr.length > 0">
                    <Button type="primary" @click.prevent="cancelStatys('allCancel', 'yes')">添加资格</Button>
                </FormItem>
                <FormItem label="发放日期">
                    <Select @on-change="changeDate" v-model="searchKey.date" clearable>
        ...
        <div class="v2-search">
            <Form inline>
                <FormItem v-for="(t, i) in arrTime" :key="i">
                    <Button class="ft16" :class="searchKey.wave == t.value ? 'bd-bg' : 'bg-or'" @click.prevent="changeTime(t)">{{ t.name }}</Button>
                </FormItem>
            </Form>
        </div>
        <div class="v2-table">
        ...
        <Modals title="设置金额" ref="setAmount" :width="600">
            <div slot="content">
                <Form class="ft14 ml65 " :model="formKey" :rules="rules">
                    <FormItem class="mt5 mr10 "
                        >会员账号：<span style="color:black">{{ setData.userName }}</span></FormItem
                    >
                    <FormItem class="mt5 mr10"
                        >历史存款：<span style="color:black">{{ setData.pay_money }}</span></FormItem
                    >
                    <FormItem class="mt5 mr10"
                        >历史打码：<span style="color:black">{{ setData.bet_money }}</span></FormItem
                    >
                    <FormItem class="mt5 mr10 iconColor"
                        >昨日打码：<span style="color:black">{{ setData.yesterday_bet_money }}</span></FormItem
                    >
                    ...
                    <FormItem class="mt20 mb20 ml50">
                        <Button type="primary" class="ivu submit" @click="save">确定</Button>
                    </FormItem>
                </Form>
            </div>
        </Modals>
        <Modals title="等级对照表" ref="levelChat" :width="897">
            <div slot="content" class="ml40 mr40 modal-overflow">
                <div class="v2-footer mb10 ft14" style="height:51px;line-height:51px">
                    ...
                    <span class="ml10 mr10"
                        >本日计划：
                        {{ Number(totalLevelData.today_plan).toFixed(2) }}
                    </span>
...
export default {
            ...
            table: {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        key: 'date_time',
                        render: (h, params) => {
                            const { requirement_status } = params.row;
                            let color = requirement_status == 'yes' ? '#2d8cf0' : '#ff2626';
                            if (this.btnName == '发放表') {
                                if (requirement_status == 'yes' || requirement_status == 'no')
                                    return (
                                        <span
                                            class="blue-font"
                                            onClick={() => {
    ...
    methods: {
        ...
        changeTime(t) {
            const { name, value } = t;
            if (!this.searchKey.level) {
                this.searchKey.level = 'all';
            }
            this.searchKey.wave = value;
        ...
        cancelStatys(type, params) {
            ...
            this.$Tip.confirm({
                content: text,
                onOk: () => {
                    redPacketUpStatus(this.data, true).then(res => {
                        if (type == 'oneCancel') {
        ...
        reduceY() {
            ...
            for (let key in data) {
                ...
                if (key == 'level' || key == 'sort' || key == 'sortval' || key == 'table') {
                    delete data[key];
                }
            }
            this.disabled = false;
        ...
        chart(type) {
            ...
            if (name == '资格表') {
                ...
                this.selection = { type: 'selection', align: 'center' };
                if (JSON.stringify(this.table.columns).includes('selection')) {
                    return false;
                } else {
                    if (this.changeNumber == 1) {
                        arr.unshift(this.selection);
                    }
                }
        ...
        getSelectList(data) {
            redPacketGetSelect(data).then(res => {
                if (res != undefined && res.code == 200) {
                    ...
                    const { data, time } = red_packet_rain;
                    for (let key in time) {
                        this.arrTime = time[key];
                    }
