    ...
    <div>
        <div class="v2-search clearfix mb10">
            <Form inline>
                ...
                <div class="fl">
                    <Button class="cardNutton fr ml20 activity-btn bc-c1 w100x fontColor" @click="selectWay('mobile')" :class="searchKey.client_type === 'mobile' ? 'active' : ''"
                        >移动端</Button
                    >
                    <Button class="cardNutton fr ml20 activity-btn bc-c1 w100x fontColor" @click="selectWay('pc')" :class="searchKey.client_type === 'pc' ? 'active' : ''"
                        >PC</Button
                    >
                    <Button
                        class="cardNutton fr ml20 activity-btn bc-c1 w100x fontColor"
                        @click.prevent="selectWay('')"
                        style="line-height:36px"
        ...
        <div class="v2-search bb mb20 pt5">
            <ButtonGroup class="repay-button">
                <Button class="fontColor" @click.prevent="getall()" :class="currentBtn == 'all' ? 'active' : 'noactive'">
                    全部
                </Button>
                <Button
                    class="fontColor"
                ...
                >
                    ...
                    <i v-if="adminType == 'super' || authInPage.indexOf('activityCateDel') != -1" class="ivu-icon ivu-icon-ios-close" @click.stop="delactity(item)"></i>
                </Button>
      ...
      -->
            <Modals ref="ModalsDetial" title="预览地址" :width="previewlist.activity_type === 'url_activity' ? '760px' : '895px'">
                ...
                <div
                    ...
                    style="width:auto;font-size:1rem;color:#000;margin:8px;font-family:sans-sefiry"
                    v-else-if="previewlist.activity_type === 'normal_activity'"
                >
        ...
        <Modals ref="addfenleiModal" :title="updActivity === '' ? '新增分类' : '修改分类'" width="500px" @modalVisible="initActivityStatus">
            <div slot="content" class="modal-overflow" style="width:500px">
                <Form ref="memberLevelForm" :rules="addActivityRules" :label-width="150" :model="memberLevelForm">
                    <FormItem label="分类名称" prop="fenleiName">
                        <Input placeholder="请输入分类名称" class="w260x" v-model="memberLevelForm.fenleiName" />
                    </FormItem>
                    <Button class="ivu-btn addSave mt40" @click.prevent="updActivity === '' ? addsave() : updActivityCate()" :loading="$root.editBtnLoading">{{
                        updActivity === '' ? '确认添加' : '确认修改'
                    }}</Button>
                </Form>
            </div>
        </Modals>
...
export default {
    ...
    methods: {
        ...
        preview(params) {
            //预览
            activityDetail({ siteId: this.$root.nowSite.id, id: params.row.id }).then(res => {
