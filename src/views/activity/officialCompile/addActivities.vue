<template>
    <div class="v2-search" :key="$route.meta.title">
        <Form ref="formKey" :label-width="80" :model="formKey" :rules="rules">
            <FormItem style="margin-bottom:20px !important" label="活动类型">
                <Select v-model="formKey.activity_type" class="w419x">
                    ...
                    <Option value="apply_activity" v-if="siteId == 22 || siteId == 42">报名活动</Option>
                </Select>
            </FormItem>
            ...
            <div class="mb20">
                <FormItem label="显示状态">
                    ...
                    <span class="ml30" style="vertical-align:middle" v-if="siteId != 22 && siteId != 42">跳转按钮</span>
                    <Switch class="ml20" v-model="formKey.jump_switch" size="large" style="vertical-align:text-top" v-if="siteId != 22 && siteId != 42">
                        <span slot="open">开启</span>
                        <span slot="close">停用</span>
                    </Switch>
                       <span class="ml30" style="vertical-align:middle">显示横幅</span>
                    <Switch class="ml20" v-model="formKey.is_show_banner" size="large" style="vertical-align:text-top">
                        <span slot="open">开启</span>
                        <span slot="close">停用</span>
                    </Switch>
                    <span class="ml30" style="vertical-align:middle">显示内容</span>
                    <Switch class="ml20" v-model="formKey.is_show_content" size="large" style="vertical-align:text-top">
                        <span slot="open">开启</span>
                        <span slot="close">停用</span>
                    </Switch>
                </FormItem>
            ...
            <FormItem label="标题图片" prop="image" class="pic">
                ...
                <div class="mb mb20 ml5" :style="formKey.client_type.length < 2 ? { textAlign: 'center' } : {}">
                    ...
                    <div class="upload-img">
                        <UpLoads
                            v-for="item in [
                                { ...this.uploadForm, showType: 'pc' },
                                { ...this.uploadForm, showType: 'mobile' }
                            ]"
                            v-bind="item"
                            :key="item.showType"
                ...
                <div class="mb20 add-acitivity" v-show="seltext" key="txt">
                    <div v-for="item in ['pc', 'mobile']" :key="item">
                        ...
                        <!-- <kindeditor
                            ...
                            :id="item === 'pc' ? ue1 : ue2"
                            :ref="item === 'pc' ? ue1 : ue2"
                            :content="item === 'pc' ? defaultMsg1 : defaultMsg2"
                            :config="config"
                            :key="item === 'pc' ? ue1 : ue2"></kindeditor> -->
                    </div>
                </div>
            </div>
        ...
        <Button class="ivu-btn addSave ml100 mt20" :style="formKey.activity_type == 'apply_activity' ? 'margin-top:60px' : ''" @click.prevent="submit">{{
    ...
    data() {
        ...
        return {
            ...
            rules: {
                ...
                offline_time: [
                    { required: true, message: '请选择结束日期', trigger: 'blur', pattern: /.+/ },
    ...
    methods: {
                ...
                if (valid) {
                    ...
                    if (this.formKey.activity_type == 'normal_activity') {
                        if (this.formKey.client_type.includes('pc') && this.formKey.client_type.indexOf('mobile') == -1) {
                            if (this.$refs.ue1[0].getUEContent() == '' && !this.formKey.description.pc) {
                                this.$Message.error('请输入PC端内容');
                                return false;
                            }
                        }
                        if (this.formKey.client_type.includes('mobile') && this.formKey.client_type.indexOf('pc') == -1) {
                            if (this.$refs.ue2[0].getUEContent() == '' && !this.formKey.description.mobile) {
                    ...
                    data.is_show_banner=data.is_show_banner==true?'yes':'no';
                    data.is_show_content=data.is_show_content==true?'yes':'no';
        ...
        initEdit(obj) {
            Object.keys(this.formKey).forEach(el => {
                ...
                }else if(el === 'is_show_banner'){
                     this.formKey[el] = obj[el] === 'yes' ? true : false;
                }else if(el ==='is_show_content'){
                    this.formKey[el] = obj[el] === 'yes' ? true : false;
