    ...
    <div class="v2-search" :key="$route.meta.title">
        <Form ref="formKey" :label-width="80" :model="formKey" :rules="rules">
            ...
            <div v-if="formKey.activity_type == 'url_activity'" class="mt40">
                <div v-for="item in ['pc', 'mobile']" :key="item" class="mb20">
                    <FormItem :label="`${item === 'pc' ? 'PC' : '移动端'}地址`" class="mb20 mt20" v-if="formKey.client_type.includes(item)">
        ...
        <Button class="ivu-btn addSave ml100 mt20" @click="submit">{{ $route.params.page === 'edit' ? '确认保存' : '确认新增' }}</Button>
