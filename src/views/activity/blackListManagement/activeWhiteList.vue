  ...
  <div>
    <div class="v2-search">
      <Form ref="searchKey" inline>
        <FormItem>
          <Button
            :class="!disabled ? 'bb-color blue-bd mr20' : 'mr20'"
            v-show="!disabled"
            @click.prevent="batchOut"
            ><span>移出白名单</span></Button
          >
          <Button
            class="bb-color blue-bd"
            @click.prevent="showModals('addBlackList')"
            >添加白名单</Button
          >
        </FormItem>
        <!-- <FormItem label="活动类型">
          <Select
            filterable
            v-model="searchKey.activety_name"
            class="s-120"
            clearable
            placeholder="全部"
          >
            <Option v-for="(t, i) in activityList" :key="i" :value="t.name">{{
              t.name
            }}</Option>
          </Select>
        </FormItem> -->
    ...
    <!-- 添加白名单 -->
    <Modals ref="addBlackList" title="添加活动白名单" width="820">
        ...
        >
          ...
          <!-- <FormItem class="clearfix" v-if="selectData.length > 0">
            <div style="margin-left: -75px; float: left">
              <span class="red-font">
                {{ selectData.length > 0 ? "*" : "" }}</span
              >
              <span class="mr2">
                {{ selectData.length > 0 ? "活动类型" : "" }}</span
              >
            </div>
            ...
            <div
              ...
              :style="
                selectData.length > 18 ? 'height:185px;overflow-y:auto' : ''
              "
            >
              <span
                style="border-radius: 5px; height: 30px; line-height: 30px"
                v-for="(item, index) in selectData"
                :key="index"
                class="ml5 ib b pl5 pr5 mr10 mt3 mb3"
              >
                {{ item.name }}
                <Icon
                  type="ios-close"
          ...
          <!-- <FormItem class="clearfix">
            ...
            <div v-if="searchData.length > 0" class="b mt5">
              <checkbox
                class="ml5"
                @on-change="(val) => checkOne(t, val)"
                v-model="t.checked"
                v-for="(t, index) in searchData"
                :key="index"
              >
                <span>{{ t.name }}</span>
              </checkbox>
          ...
          <FormItem class="mb20">
            <Button
              ...
              style="width: 520px"
              class="bg ft20 ivu-btn commonSubmit"
              @click.prevent="submit('add')"
              >确认添加</Button
            >
          </FormItem>
        </Form>
    ...
    <Modals ref="editBlackList" width="820" title="编辑活动白名单">
      <div slot="content">
        <Form
          ref="editBlackInfo"
          :model="editBlackInfo"
        ...
        >
          ...
          <!-- <FormItem class="clearfix">
            ...
            <div
              ...
              v-if="selectData.length > 0"
              :style="
                selectData.length > 20 ? 'height:175px;overflow-y:auto' : ''
              "
            >
              <span
...
export default {
    ...
    return {
      ...
      table: {
        ...
        columns: [
          ...
          //             });
          //           }
          //           this.editBlackInfo.activety_name =
          //             params.row.activety_name &&
          //             params.row.activety_name.split(",");
          ...
          //                 ids: params.row.id,
          //               };
          //               removeWhiteList(data, true).then((res) => {
      ...
      rules: {
        userNames: [
          { validator: validateUserNames, required: true, trigger: "blur" },
        ],
      },
    };
  },
  ...
  methods: {
    ...
    //     whiteListShow({params:data}).then(res=>{
    //         if(res!=undefined&&res.code==200){
    ...
    submit(stage) {
      if (stage == "add") {
        this.$refs.addBlackInfo.validate((valid) => {
          if (valid) {
            this.$root.startEditLoading();
            this.addBlackInfo.userNames = this.addBlackInfo.userNames.replace(
              / /g,
              ""
            );
              ...
              .then((res) => {
                ...
                if (res != undefined && res.code == 200) {
                  const {
                    in_blacklist,
                    not_exists,
                    already_exists_user_activety_list,
                  } = res.data;
                  let userNameContentB,
                    activetyNameB,
                    userNameCountB,
                    blackContent,
                    blackCount,
                  ...
                  if (
                    already_exists_user_activety_list &&
                    already_exists_user_activety_list.length > 0
                  ) {
                    const {
                      userName,
                      activety_name,
                    } = already_exists_user_activety_list[0];
                    activetyNameB = activety_name;
                    userNameCountB = userName.length;
                    userNameContentB = userName.toString();
                  }
                  if (in_blacklist) {
                    blackContent = in_blacklist.toString();
                    blackCount = in_blacklist.length;
                  }
                  if (not_exists) {
                    noBlackContent = not_exists.toString();
                    noBlackCount = not_exists.length;
                  }
                  ...
                  if (
                    ...
                    this.$Message.success({ content: "添加白名单成功" });
                  } else {
                    // blackCount
                    if (userNameCountB > 0) {
                      this.$Tip.showAddSub({
      ...
      if (stage == "edit") {
        ...
        //   return false;
        // }
        whiteListEdit(data, true).then((res) => {
          if (res != undefined && res.code == 200) {
            const {
              in_blacklist,
              not_exists,
            ...
            if (
              (in_blacklist == 0 && not_exists == 0 && userNameCountB == 0) ||
              (in_blacklist == undefined &&
                not_exists == undefined &&
                userNameCountB == undefined)
            ) {
