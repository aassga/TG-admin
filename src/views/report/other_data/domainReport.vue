...
export default {
  ...
  computed: {
    ...mapState({
      domainLevelList: (state) => state.report.domainLevelList,
    }),
  },
  data() {
    return {
      ...
      table: {
        ...
        columns: [
         
          {
            ...
            render: (h, params) => {
              ...
              if (domain == "总计") {
                ...
                return (
                  ...
                  >
                    {params.row.domain.split(":")[0]?params.row.domain.split(":")[0]:'-'}
                  </span>
                )
                else{
                  return(
          ...
          },
           {
            ...
            render: (h, params) =>{
                const { domain } = params.row;
                if(domain!='总计'){
                 return <span>{params.row.level==0?'未分层':'第'+this.$root.ArabiSimplified(params.row.level)+'层'}</span>
                }else{
                    return(<span>-</span>)
                }
          ...
          },
          {
            ...
            render: (h, params) => {
              let color =
                params.row.isSSL == "是" || params.row.isSSL == "-"
                  ? ""
                  : "red";
  ...
  methods: {
    ...
    searchSubmit() {
      ...
      if (this.searchKey.domain)
        this.searchKey.domain = this.$root.trimAll(this.searchKey.domain);
      let data = {
        ...this.searchKey,
    ...
    exportExcel() {
      ...
      domainReports({ params: data }, true)
        .then((res) => {
