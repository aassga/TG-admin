...
import { operationHourCompareReport } from "@/api/report";
...
export default {
            ...
            table: {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        minWidth: 60,
                        render: (h, params) => {
                            return <span>{params.row.data_hour}</span>;
                        },
                    },
                    {
                        title: "IOS",
    ...
    beforeRouteEnter: (to, from, next) => {
        let isFromHour = from.name === 'operationHourReport'
        next((vm) => {
            vm.isFromHour = isFromHour
            const params = {
                params: {
                    ...vm.$route.query
                }
    ...
    mounted() {
        ...
        window.onpopstate = function(event) {
            ...
            if (!vm.isFromHour) {
                ...
                if (operationHourReportRoute.length > 0) {
                    vm.$router.replace({ name: operationHourReportRoute[0].name, query: operationHourReportRoute[0].query }).catch(() => {})
                } else if (operationDayRoute.length > 0) {
    ...
    methods: {
        getData(data) {
            ...
            operationHourCompareReport(data)
                .then((res) => {
                    if (res.data.data) {
                        const { data } = res.data;
        ...
        exportExcel() {
            ...
            operationHourCompareReport({ params: data })
                .then((res) => {
        ...
        goBack() {
            const operationHourCompareReportRoute = this.tagNavList.filter(item => item.name === 'operationHourCompareReport')[0]
            const operationHourReportRoute = this.tagNavList.filter((item) => item.name === 'operationHourReport')[0]
            this.$router.push({
                name: operationHourReportRoute.name,
                query: {
                    ...operationHourReportRoute.query
                }
            })
            this.$store.commit('closeTag', operationHourCompareReportRoute);
        },
        queryString(object) {
            let result = []
