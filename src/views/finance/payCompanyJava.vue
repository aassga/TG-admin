			...
			<Form ref="searchKey" inline>
				...
				<FormItem>
					<Input
						...
						:class="screen<1600&&screen>1400?'s-110':screen<1400?'s-90':'s-160'"
					/>
				</FormItem>
				<FormItem>
				...
				<div class="box">
					<div class="detail" ref="formKey" :model="formKey">
						...
						<p v-if="dayShow">
							...
							<Select
								v-if="dayShow"
								v-model="formKey.paymentBonus"
								style="width:120px;"
								@on-change="changeBonus"
							>
								<Option v-for="(item,i) in paymentBonusList" :key="i" :value="item">{{item}}%</Option>
							</Select>
						</p>
						<p>
							<label>备注信息：</label>
...
<script>
	...
	export default {
		...
		beforeRouteEnter(to, from, next) {
          ...
          const { startTime, endTime} = vm.$route.query
					vm.Time = [startTime, endTime]
					let params = { startTime, endTime, siteId: vm.$root.nowSite.id, typeTag: 'userName'}
					vm.getData({ params })
		...
		methods: {
      ...mapMutations(['setLockData_online']),
      			audit(val) {
			...
			exportExcel() {
				...
				if (this.table.total > 150000) {
					...
					depositReplaceList({ params: data })
						.then(res => {
        ...
        //查询大于7天事件判断
          ...
          return false;
			...
			success() {
				...
				if (!this.dayShow) params.paymentBonus = 0
