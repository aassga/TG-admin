...
<template>
	<div>
		...
		<Modals
			...
			title="取款详情"
			:width="870"
			@modalVisible="modalChange(arguments,lockData, operateType)"
			@closeModal="closeWithdrawal()"
			class-name="flex-item-start"
		>
			<div slot="content">
				...
				>
						...
						<div class="detail">
							...
							<p>
								...
								<Button
									...
									size="small"
									class="ml50 ivu-btn copy lh14"
									v-clipboard:copy="dataDetail.copyAccountName"
									v-clipboard:success="onCopy"
									v-clipboard:error="onError"
									@click="noPayment()"
								>复制</Button>
							...
							<p>
								<label>实际取款</label>
								<span class="font-bold" style="color:red !important">{{dataDetail.actualAmount}}</span>
								<Button
									v-if="copyButton"
									type="info"
									size="small"
							...
							<p v-if="waitModal">
								<label>操作类型</label>
								<RadioGroup
									v-if="detailModal == 'fail' && !hasSplitData"
									v-model="auditType"
								...
								<span
									v-else
									class="blueText"
								>{{ allSubWithdrawalSuccess ? '通过' : '待审核'}}</span>
							</p>
							<!-- 成功状态也会显示加入状态判断 -->
							...
							<!-- 审核详情页面展示确认提交和自动出款 -->
							<div class="txt-a clearfix" v-if="detailModal == 'fail' && waitModal && !hasSplitData">
								<Button class="moneySubmit ml20" type="primary" @click="submit" v-if="showBtn">确认提交</Button>
								<Button
									class="moneySubmit fl ml20"
									type="primary"
									@click="submit"
					...
					<Row class="pl30 pr30 mt30 tabGroup" v-if="hasSplitData">
						<Col span="6" class="tab txt-a" :class="{'active' : isDataDetailTable }">
							<div class="ft18 pt4 pb5 cs" @click="toggleModalTable('1')">拆分出款明细</div>
						</Col>
						<Col span="6" class="tab txt-a" :class="{'active' : !isDataDetailTable }">
							<div class="ft18 pt4 pb5 cs" @click="toggleModalTable('2')">近30天数据</div>
						</Col>
					</Row>
					<Tables
						v-if="hasSplitData"
						v-show="isDataDetailTable"
						class="pt10 ml30"
						:width="810"
						:columns="tableDataDetail.columns"
						:value="tableDataDetail.data"
						:loading="tableDataDetail.loading"
						:page="tableDataDetail.page"
					></Tables>
					<Tables
						v-show="!isDataDetailTable || !hasSplitData"
						class="pt10 ml30"
						:width="810"
				...
				<div
					style="height:685px"
					v-if="nextStatus && ((adminType == 'super') || (authInPage.indexOf('withdrawalsDetail') != -1))"
				>
					<Tables
	...
	export default {
		...
		computed: {
			...mapState({
				...
				lockData: state => state.info.lockData
			}),
			islastSubWithdrawal() {
				return this.tableDataDetail.data.filter((item) => item.status !== 'success').length === 1
			},
			allSubWithdrawalSuccess() {
				return this.tableDataDetail.data.filter((item) => item.status !== 'success').length === 0
			}
		},
		data() {
			return {
				...
				isDataDetailTable: false,  // 取款詳情表格類別控制
					...
					columns: [
						...
						},
						{
							...
							render: (h, params) => {
								...
								} else if (params.row.status === 'balanceNotEnough') {
									return (
										<span>待發送</span>
									)
								} else {
								...
								if (params.row.status === 'success') {
									...
									return (
										<div>
											...
											<a
												onClick={() => {
													this.retrySubStatus(params.row)
												}}
											>
												补单
											</a>
				...
				table: {
					...
					columns: [
						...
						},
						{
							...
							minWidth: 70,
							render: (h, params) => {
								return <span class="red-font">{params.row.actualAmount}</span>;
							}
						},
						{
							title: "状态",
									...
									<div>
										<Button
											...
											onClick={() => {
												...
												console.log('this.moneyStatus',this.moneyStatus)
      ...
      ...mapMutations(["setLockData"]),
			...
			exportExcel() {//导出
				...
				if (this.table.total > 150000) {
					...
					withdrawalsListData({ params: data },true)
						.then(res => {
			...
			batchExport(index) {
				...
				this.$root.startEditLoading();
				let openr = window.open();
				withdrawalsListData({ params: data })
					.then(res => {
						if (res != undefined && res.code == 200) {
							let region = res.data.downloadUrl;
							let nowHost = region.split(".com")[1];
			...
			toggleModalTable(type) {
				if (type === '1') {
					this.isDataDetailTable = true
				} else {
					this.isDataDetailTable = false
				}
			},
			//审核的弹框  详情的弹框
			...
			//详情的列表
			getDetailData(row) {
				const { id, time, remark,status } = row;
				//  拒绝原因
				this.time = time;
          ...
          data.type='detail'
				...
				withdrawalsDetail(data)
					.then(res => {
						...
						this.dataDetail.copyAccountName = this.dataDetail.bankAccountName.includes(
							"_"
						)
							? this.dataDetail.bankAccountName.split("_")[0]
							: this.dataDetail.bankAccountName;
						//表格数据
						...
						if (res.data.subOrder) {
							this.hasSplitData = true;
							this.isDataDetailTable = true;
							this.tableDataDetail.data = res.data.subOrder
						}
						this.$refs.auditModal.spinHide();
					})
					.catch(err => {
			...
			async autoRetryAllSubStatus() {
				let hasError = false;
				const { id, time } = this.table.data[this.now_index]
				const detailPayload = {
					id,
					timestamp: time,
					siteId: this.$root.nowSite.id,
				...
				const detail = await withdrawalsDetail(detailPayload, true)
				const { subOrder } = detail.data
				for (const item of subOrder) {
					const payload = {
						siteId: this.$root.nowSite.id,
						timestamp: this.time,
						status: 'success',
						withdrawalSubId: item.id,
					}
					const result = await withdrawalsSubAudit(payload, true, 0, true)
					if (result.code === 400 && result.message !== '馀额不足') {
						hasError = true
					}
				}
			...
			editSubStatus(row) {
				let data = {
					...
					timestamp: this.time,
					status: 'success',
					withdrawalSubId: row.id,
				}

				this.$Tip.confirm({
					content: "您确定此笔订单已出款成功吗？",
					onOk: () => {
						this.tableDataDetail.loading = true
						withdrawalsSubEdit(data).then((res) => {
							this.refreshDetailData()
							if (res != undefined && res.code == 200) {
			...
			retrySubStatus(row) {
				...
				this.$Tip.confirm({
					...
					onOk: () => {
						this.tableDataDetail.loading = true
						withdrawalsSubAudit(data).then((res) => {
							if (res != undefined && res.code == 200) {
								this.$Message.success(res.data);
							} else {
			...
			async refreshDetailData() {
				const { id, time, status } = this.table.data[this.now_index];
				...
				if (status === 'wait' && !this.islastSubWithdrawal) {
          data.type='audit'
				} else {
          data.type='detail'
				}

				await withdrawalsDetail(data).then(async (res) => {
					this.hasSplitData = false;
					if (res.data.subOrder) {
						this.hasSplitData = true;
						this.isDataDetailTable = true;
					...
					this.tableDataDetail.loading = false
				})
			},
			//将最后一栏固定在底部
			scrollToBottom() {
			...
			modalChange(value, lockData, stage) {
				let flag = value[0];
      ...
      loanSubmit(){//自动出款按钮
					this.$refs.SendLoanModal.showModal();
					...
					isAutomaticWithdrawal: "yes",
					automaticWithdrawalId :this.automaticWithdrawalId
				};
			...
			submit() {
				...
				withdrawalsAudit(datatail, true).then(res => {
					if (res != undefined && res.code == 200) {
