<template>
    <div>
        <Select
            ...
            @on-change="userClick"
        >
            <Option v-for="(item, i) in siteList" :value="item.value" :key="i">{{ item.name }}</Option>
        </Select>
        <Select
            v-else
            multiple
        ...
        singleChange(val) {
            this.siteList.forEach(s => {
                if (s.id == val.id) {
                    ...
                    // this.singleData.paymentBonus = s.paymentBonus
                ...
                val.id.forEach(item => {
                    this.siteList.forEach(s => {
                        if (s.id == item) {
                            ...
                            // this.multipleData.paymentBonus.push(s.paymentBonus);
                            this.saveSite(s.id);
                        }
                    });
                });
