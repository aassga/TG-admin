    ...
    <div>
        <div class="v2-search clearfix">
            <Form ref="searchKey " class="fl" inline>
                <FormItem label="开始时间">
                    <DatePicker type="datetime" placeholder="开始日期" class="s-185" v-model="Time[0]" :options="options3" format="yyyy-MM-dd HH:mm:ss"></DatePicker>
                </FormItem>
                <FormItem label="结束时间">
                    <DatePicker type="datetime" placeholder="结束日期" class="s-185" v-model="Time[1]" :options="options3" format="yyyy-MM-dd HH:mm:ss"></DatePicker>
                </FormItem>
                <FormItem label="会员账号">
                    <Input v-model="searchKey.userName" @keyup.enter.native="searchSubmit" placeholder="请输入会员账号" class="s-140" />
                </FormItem>
            </Form>
            <div class="fl ml20"> <Button type="primary" :loading="$root.tableLoading" class="search" @click.prevent="searchSubmit">查询</Button></div>
        </div>
        <div class="v2-table">
            <Tables ref="table" :loading="$root.tableLoading" :columns="table.columns" :value="table.data" :page="false" @on-sort-change="sortChange"></Tables>
        </div>
...
export default {
    name: 'auditOne',
    components: { Tables, GameSelect, Icons, summaryDataShow },
    data() {
        return {
            isShow:true,
            flag: false,
                ...
                columns: [
                                    ...
                                    class={{
                                        'blue-font':
                                            ...
                                            (this.adminType == 'super' ||
                                                (this.authInPage.indexOf('platformReportsBySingle') != -1 && this.authInPage.indexOf('gameRecordsDetail') != -1))
                                    }}
                                    onClick={() => {
                                        if (
                                            this.adminType == 'super' ||
                    ...
                    },
                    {
                        ...
                        render: (h, params) => {
                            ...
                            return <span style={{ color }}>{winRate}%</span>;
                        }
                    }
                ]
            }
    ...
    beforeRouteEnter(to, from, next) {
        next(vm => {
            ...
            for (let key in gameSearchkey) {
                if (!gameSearchkey[key]) delete gameSearchkey[key];
            }
    ...
    methods: {
        getData(data) {
            ...
            platformReportsBySingle(data).then(res => {
                ...
                this.isShow = this.table.data == 0?false:true;
        ...
        searchSubmit() {
            ...
            if (new Date(this.Time[1]).getTime() - new Date(this.Time[0]).getTime() > 2678400000) {
                this.$Tip.info({ content: '最多只能查询31天' });
                return false;
            }
