  <template>
	<div>
		...
		<Modals
			...
			title="入款详情"
			:width="760"
			@modalVisible="modalChange(arguments, lockData_income)"
		>
			<div slot="content">
				<div class="box">
					<div class="detail" ref="formKey" :model="formKey">
						...
						<p>
							<label>交易的ID：</label>
							<span>{{dataDetail.bankSerialNumber}}</span>
						</p>
						<p>
							<label>存入账户：</label>
							<span>{{dataDetail.subType}}/{{dataDetail.bankAccountName}}</span>
						</p>
						<p>
							<label>收款地址：</label>
							<Tooltip placement="bottom" transfer>
								<span>.....{{String(dataDetail.bankId).substring(String(dataDetail.bankId).length-6)}}</span>
								<div slot="content">{{dataDetail.bankId}}</div>
							</Tooltip>
						</p>
						<p>
							<label>货币数量：</label>
							<span class="red-font font-bold">{{Number(dataDetail.usdt_count).toFixed(2)}}</span>
						</p>
						<p>
							<label>存款金额：</label>
...
<script>
	...
	export default {
		...
		computed: {
			...mapState({
				autoFresh: state => state.info.virtualIncomeFresh,
				autoFreshTimer: state => state.info.virtualIncomeTimer,
				lockData_income: state => state.info.lockData_income,
				preferentialConfig: state => state.app.preferentialConfig,
				depositPreferentialType: state => state.app.depositPreferentialType
			})
		},
		data() {
			return {
				...
				table: {
					...
					columns: [
						...
						},
						{
							...
							render: (h, params) => (
								<div>
									...
									<p class="pt1 fh">{params.row.bankSerialNumber}</p>
								</div>
							)
						},
						{
						...
						},
						{
							...
							minWidth: 45,
							render:(h,params) => {
								return <span >{Number(params.row.usdt_rate).toFixed(2)}</span>;
							}
						},
                        {
                            title: "货币数量",
							...
							render:(h,params)=>{
								return(
									<span class="red-font">{Number(params.row.usdt_count).toFixed(2)}</span>
								)
							}
                        },
						{
							...
							tooltip: true,
							render: (h, params) => {
								let len = params.row.bankId.length;
								return(
									<div>
										<p class="bb pb2 fh">{params.row.bankAccountName}</p>
										<Tooltip placement="bottom" max-width="800" transfer>
											<p class="pt1 fh">{'...'+params.row.bankId.substring(len-6)}</p>
											<div slot="content">{params.row.bankId}</div>
										</Tooltip>
									</div>
								)
							}
									...
									"div",
									{
										style: {
											display:
												...
												this.authInPage.indexOf("depositIncomeAudit") != -1
													? "block"
													: "none"
										}
									},
												...
												on: {
													click: () => {
														...
														if(this.preferentialConfig){
															let arr=this.preferentialConfig[1].config;
															let arrStatus=this.preferentialConfig[1].status;
															if(arr.length!=0){
															  ...
															  if(arrStatus=='off'){	
																...
																let newobj = {}; 
																 m= m .reduce((preVal, curVal) => {newobj[curVal.preferential] ? '' : newobj[curVal.preferential] = preVal.push(curVal); return preVal }, [])
                                                                 this.paymentBonusList=m;
															}else{
																this.paymentBonusList=[{codeMultiples:"1",preferential:"2",}];
																this.formKey.paymentBonus="2";
															}
														}else{
														    this.formKey.paymentBonus="2";
															this.paymentBonusList=[{codeMultiples:"1",preferential:"2",}]
															...
															this.DepositMoney = (this.dataDetail.amount *(1 + this.formKey.paymentBonus / 100)).toFixed(2);
															//锁定
															...
															auditLock(data).then(res => {
																if (res.data && res.data == true) {
																	this.setLockData_income(params.row)
		...
		beforeDestroy() {
			this.changeFresh({ stage: "virtualIncome", val: this.autoFresh });
			this.clearTime("virtualIncome");
		},
		watch: {
			nowSite: {
				handler(val, oldVal) {
					this.$nextTick(() => {
						...
						name === "virtualIncome" && this.$refs.LevelSelect.init(this.$root.nowSite.id);
					});
				},
				deep: true,
				immediate: true
		...
		created() {
				// console.log(this.preferentialConfig)
				// if(this.preferentialConfig){
				// 	console.log(111)
				// }else{
				// 	console.log(222)
			...
			if (type == 1) {
				if (Object.keys(this.lockData_income).length > 0) {
					const { id, time } = this.lockData_income
					const data = {
						id,
						siteId: this.$root.nowSite.id,
						type: 'Deposit',
		...
		methods: {
			...mapMutations(['setLockData_income']),
			...
			exportExcel() {
				...
				if (this.table.total > 150000) {
					...
					depositVirtualList({ params: data },true)
						.then(res => {
			...
			batchExport(index) {
				...
				this.$root.startEditLoading();
				let openr = window.open();
				depositVirtualList({ params: data })
					.then(res => {
						if (res.code == 200) {
			...
			//时间刷新
			changeAuto(val) {
				this.changeFresh({ stage: "virtualIncome", val }); //改变时间
			...
			modalChange(value, lockData_income) {
				let flag = value[0];
				if (!flag && this.count == 0) {
					//点击左上角关闭
					const { id, time } = lockData_income;
					const data = {
						id,
						siteId: this.$root.nowSite.id,
						timestamp: time,
			...
			getStoreBankNQRCode() {
				...
				virtualSearchList(data).then(res => {
