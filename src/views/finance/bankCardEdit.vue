...
<!-- v-if="String(chessList)&&String(chessList).includes($root.nowSite.id)" -->
            <span >
            ...
            <!-- prop="quick_amount"  -->
             <FormItem label="常用金额" v-if="formKey.inCard != 'transfer_account'">
                <Input class="w275x" :disabled="editDisabled" type="text" v-model="formKey.quick_amount" placeholder="客户端快捷按钮,多个以英文逗号隔开" />
            </FormItem>
           </span>
            <FormItem prop="cardAddress" label="开户网点" v-if="formKey.inCard == 'card'" >
               <Input v-model="formKey.cardAddress" :disabled="editDisabled"  placeholder="请输入开户网点" class="w275x"/>
            </FormItem>
            <FormItem label="自定义" v-if="formKey.inCard == 'card'" >
               <Input v-model="formKey.customize"  placeholder="填写后横版app读此内容不显示开户网点" class="w275x" :disabled="editDisabled" />
            </FormItem>
    ...
    data() {
        ...
        return {
            ...
            formKey: {
                ...
                min_amount: '',
                max_amount: '',
                quick_amount: '100,300,500,800,1000,3000,5000,8000,10000,30000,50000,100000',
                cardAddress: '',
                customize:'',
                id: '',
                src: '',
            ...
            rules: {
                ...
                //         validator: (rule, val, cb) => {
                //             console.log(val)
                //             // formKey.inCard != 'card'&&formKey.inCard != 'transfer_account' && formKey.inCard != 'virtual'
                //             if(this.formKey.inCard != 'virtual'&&this.formKey.inCard != 'card'){
    ...
    created() {
        ...
        this.$nextTick(() => {
            ...
            if (Object.keys(this.$route.query).length) {
                ...
                //接受点击编辑银行卡的值
                let { levelLabel, levelAlias, terminal, devices,device,group_id,group_name,quick_amount} = this.formKey;
                console.log(quick_amount)
                ...
                if (valid) {
                    ...
                    if (this.$route.params.page == 'create') {
                        ...
                        if (params.inCard == 'card') {
                           ...
                           if(!params.customize){ params.customize=''};
