  ...
  <div>
    <div class="v2-search">
      <Form ref="searchKey" inline>
        ...
        <FormItem label="站点名称">      
           <Select  class="w120x" v-model="searchKey.selectSiteId" v-if="this.singleSite=='multiple'">
             <Option v-for="(t,i) in siteCollect" :key="i" :value="t.siteId">{{t.siteName}}</Option>
           </Select>
           <span v-else >{{this.$root.nowSite.siteName}}</span>
        </FormItem>
        <FormItem label="会员账号">
      ...
      <div slot="content" class="v2-form-wrapper">
        <Form ref="addBlack" :label-width="160" :rules="rules" :model="addBlack" style="width:570px;" >
          <FormItem label='站点名称' prop='siteId' v-if="title == '添加黑名单'">
            ...
            <Select v-model="addBlack.siteId" v-if="this.singleSite=='multiple'" class="w260x" placeholder='关键字查询' filterable clearable>
              <Option v-for="(t,i) in siteCollect" :key="'site' + i" :value="t.siteId">{{t.siteName}}</Option>
            </Select>
          ...
          <FormItem label="会员账号" class="mt20 mb30" v-if="title == '添加黑名单'">
            <AutoSearch v-if="title == '添加黑名单'" :disabled='!addBlack.siteId' :siteId='addBlack.siteId' @sendSuperior="getUsername" ref="autoSearch" :placeholder="'会员账号，默认查询'" :stage="'all'" ></AutoSearch>
          </FormItem>
...
export default {
  ...
  mounted() {
    ...
    let data = {...this.searchKey,selectSiteId:'0'};
    for (let key in data){
