          ...
          <TabPane :label="typeId=='10004'?'棋牌游戏平台':typeId=='10005'?'捕鱼游戏平台':'电子游艺平台'" key="1">
            ...
            <div class="mb20" v-if="electronicList.length">
              ...
              <div class="mb20 gameListBox" 
                ...
                @drop="currentWire==currentType+'_0'&&currentType==electronicList[currentPlatform].allGameKey?''
              ...
              >
                ...
                >
                  ...
                  ></Icons>
                  <Icon
                    v-show="isEdit&&(adminType=='super'||!item.super)&&currentWire!=(electronicList[currentPlatform].allGameKey+'_0')"
                    class="close cs"
                    style="top:6px, right:3px"
                    @click.stop="remove(item.name,electronicList[currentPlatform].list[currentType].list[currentWire].list,'游戏',i)"
                    type="md-close"
                  />
                  <Icon
                    v-show="isEdit&&typeId=='10001'&&(adminType=='super'||!item.super)&&currentWire==electronicList[currentPlatform].allGameKey+'_0'"
                    type="md-create"
                    class="bianji cs"
                    @drop.native.stop="menuItemDrop($event,i,'游戏',(electronicList[currentPlatform].list[currentType].list[currentWire].list))"
                    style="top:26px !important"
            ...
            <!-- 平台全部游戏 -->
            <div v-if="currentWire!=(electronicList[currentPlatform].allGameKey+'_0')&&isEdit">
              ...
              <div class="mb20 gameListBox" 
                  ...
                  v-if="isEdit&&electronicList[currentPlatform].list[electronicList[currentPlatform].allGameKey].list[electronicList[currentPlatform].allGameKey+'_0'].list.filter(item=>item.name.includes(text2)).length!=0">
                <div
                  v-if='i>=minNum4&&i<=maxNum4'
                  class="w220x txt-a po-re ib h60"
                  v-for="(item,i) in electronicList[currentPlatform].list[electronicList[currentPlatform].allGameKey].list[electronicList[currentPlatform].allGameKey+'_0'].list.filter(item=>item.name.includes(text2))"
                  :key="i"
                >
                  <Button
            ...
            <!-- <div v-if="electronicList[currentPlatform].list[electronicList[currentPlatform].allGameKey].list[electronicList[currentPlatform].allGameKey+'_0'].list.filter(item=>item.name.includes(text2)).length==0" 
  ...
  methods: {
    ...
    getP(id){
      let msg=id=='10014'?'AE':id=='10015'?'AG':id=='10016'?'BBIN':id=='10018'?'CQ9':id=='10019'?'DT':id=='10020'?'FG':id=='10021'?'JDB':
              ...
              id=='10054'?'JDB':id=='10058'?'BG':id=='11660'?'天豪':id=='11661'?'乐游':id=='11659'?'开元':id=='11440'?'BBIN':id=='11677'?'PT':
              id=='11662'?'VG':id=='11746'?'MG':id=='11759'?'大富翁':id=='11775'?'MT':id=='11828'?'龙城':id=='11857'?'新世界':id=='11881'?'AK':
              id=='10732'?'FG':id=='11906'?'PP':id=='11751'?'皇冠':id=='11753'?'利记':id=='12034'?'WM':id=='11757'?'EVO':id=='12042'?'龙城':id=='12036'?'MT':'';
      return msg
    },
    ...
    setColor(){ //全部平台样式置灰
      if(this.activeKey=='1'){ //平台
        this.electronicList[this.currentPlatform].list[this.electronicList[this.currentPlatform].allGameKey].list[this.electronicList[this.currentPlatform].allGameKey+'_0'].list.map(item=>{
          item.color='0';
          if(this.currentWire==this.currentType+'_0'){ //全部时 置灰全部游戏
            this.electronicList[this.currentPlatform].list[this.currentType].list[this.currentWire].list.map(t=>{
      ...
      if (type == "标签") {    
        ...
        this.$Tip.confirm({
            ...
            if (this.currentType == i) {
              //当被选择项删除时，默认选择全部
              this.currentType = this.electronicList[this.currentPlatform].allGameKey;
              this.currentWire = this.electronicList[this.currentPlatform].allGameKey+"_0";
            }
            for(let key in list){
              if (list[key].name&&list[key].name == name) {
                delete list[key];
                this.$forceUpdate();
                this.setKey();
                return;
    ...
    setKey() {
      ...
      this.electronicList.map(item => {
        ...
        for (let t in item.list) {
          ...
          let long = 0;
          if(item.list[newKey].name=='全部游戏'){
            item.allGameKey=newKey;
          }
      ...
      if (this.i !== null) {
        if (this.dragType == type) {
          ...
          if(this.dragType=='搜索游戏'||this.dragType=='搜索标签'){
            ...
            if(type=='游戏'){
              if(this.currentType!=this.electronicList[this.currentPlatform].allGameKey){//二级分类不为全部时
                if(this.currentWire!=this.electronicList[this.currentPlatform].allGameKey+"_0"){//3级分类不为全部时
